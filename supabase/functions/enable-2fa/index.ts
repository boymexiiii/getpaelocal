...
import { encrypt } from '../_utils/encryption.ts';
...
serve(async (req) => {
  ...
  try {
    ...
    // Store the encrypted secret in the user's profile
    const { error } = await supabase.from('profiles').update({ twofa_secret: encryptedSecret }).eq('id', user_id);
    if (error) {
        ...
        await fetch(`${Deno.env.get('SUPABASE_URL')}/functions/v1/send-notification-email`, {
          ...
          body: JSON.stringify({
            ...
            data: {
              ...
              message: 'Two-factor authentication (2FA) was enabled on your account. If this was not you, please contact support immediately.',
    ...
    // Return only the plain secret to the client
    return new Response(JSON.stringify({ success: true, secret }), { status: 200, headers: corsHeaders });
  } catch (e) {
